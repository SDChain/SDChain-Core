...
struct CashSummary
{
    ...
    std::vector<std::pair<
        AccountID, SDAAmount>> sdaChanges;
    ...
    void sort()
    {
        std::sort (sdaChanges.begin(), sdaChanges.end());
    ...
    std::shared_ptr<SLE const> const& after)
{
    if (isDelete)
    {
        ...
        {
        case ltACCOUNT_ROOT:
            result.sdaChanges.push_back (
                std::make_pair (prev[sfAccount], SDAAmount {0}));
            return true;
    ...
    else
    {
        ...
        case ltACCOUNT_ROOT:
        {
            auto const curSda = cur[sfBalance].sda();
            if (!before || (*before)[sfBalance].sda() != curSda)
                result.sdaChanges.push_back (
                    std::make_pair (cur[sfAccount], curSda));
            return true;
        }
        case ltSDCHAIN_STATE:
        {
...
{
private:
    // Note differences in destroyed SDA between two ApplyStateTables.
    struct DropsGone
    {
        SDAAmount lhs;
        SDAAmount rhs;
...
countKeys (detail::CashSummary const& lhs, detail::CashSummary const& rhs)
{
    ...
    addIn (countKeys(lhs.sdaChanges,     rhs.sdaChanges));
...
bool CashDiff::Impl::rmDust ()
{
    ...
    // amount differences: sdaChanges, trustChanges, offerChanges, and
    // offerDeletions.  Rifle through those containers and remove any
    // entries that are _almost_ the same between lhs and rhs.

    // sdaChanges.  We call a difference of 2 drops or less dust.
    removedDust |= rmVecDust (lhsDiffs_.sdaChanges, rhsDiffs_.sdaChanges,
        [](SDAAmount const& lhs, SDAAmount const& rhs)
        {
        ...
        CashFilter rhsFilter, detail::ApplyStateTable const& rhs)
{
    ...
    // sdaChanges:
    setDiff (lhsDiffs.sdaChanges, rhsDiffs.sdaChanges, lhsDiffs_.sdaChanges);
    setDiff (rhsDiffs.sdaChanges, lhsDiffs.sdaChanges, rhsDiffs_.sdaChanges);
...
bool diffIsDust (STAmount const& v1, STAmount const& v2, std::uint8_t e10)
{
    ...
    // Handling SDA is different from IOU.
    if (v1.native())
    {
