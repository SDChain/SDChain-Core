        ...
        STAmount const& amount) const
{
    ...
    */
    ...
    if (isSDA(issuer) && adjustedAmt < beast::zero)
        // A calculated negative SDA balance is not an error case. Consider a
        // payment snippet that credits a large SDA amount and then debits the
...
PaymentSandbox::balanceChanges (ReadView const& view) const
{
    ...
    // get the change in SDA balance, Account == root, issuer == root, currency == SDA
...
SDAAmount PaymentSandbox::sdaDestroyed () const
{
