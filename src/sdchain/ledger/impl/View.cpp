              ...
              beast::Journal j)
{
    ...
    if (isSDA(currency))
    {
        return {sdaLiquid (view, account, 0, j)};
    }
    ...
    std::int32_t ownerCountAdj, beast::Journal j)
{
    ...
    if (fix1141 (view.info ().parentCloseTime))
    {
        ...
        auto const balance = view.balanceHook(id, sdaAccount(), fullBalance);
    ...
    {
        // pre-switchover
        // SDA: return balance minus reserve
        ...
        return view.balanceHook(id, sdaAccount(), amount).sda();
    }
}
    ...
    beast::Journal j)
{
    ...
    auto const sleSDChainState = std::make_shared<SLE>(
        ltSDCHAIN_STATE, uIndex);
    view.insert (sleSDChainState);

    auto lowNode = dirAdd (view, keylet::ownerDir (uLowAccountID),
        sleSDChainState->key(), false, describeOwnerDir (uLowAccountID), j);

    if (!lowNode)
        return tecDIR_FULL;

    auto highNode = dirAdd (view, keylet::ownerDir (uHighAccountID),
        sleSDChainState->key(), false, describeOwnerDir (uHighAccountID), j);
    ...
    sleSDChainState->setFieldU64 (sfLowNode, *lowNode);
    sleSDChainState->setFieldU64 (sfHighNode, *highNode);

    sleSDChainState->setFieldAmount (
    ...
    if (bNoSDChain)
    {
        uFlags |= (bSetHigh ? lsfHighNoSDChain : lsfLowNoSDChain);
    }
    if (bFreeze)
    {
    ...
    if ((slePeer->getFlags() & lsfDefaultSDChain) == 0)
    {
        // The other side's default is no rippling
        uFlags |= (bSetHigh ? lsfLowNoSDChain : lsfHighNoSDChain);
    }

    sleSDChainState->setFieldU32 (sfFlags, uFlags);
    adjustOwnerCount(view, sleAccount, 1, j);

    // ONLY: Create sdchain balance.
    sleSDChainState->setFieldAmount (sfBalance, bSetHigh ? -saBalance : saBalance);
...
{
    // Detect legacy dirs.
    bool        bLowNode    = sleSDChainState->isFieldPresent (sfLowNode);
    bool        bHighNode   = sleSDChainState->isFieldPresent (sfHighNode);
    std::uint64_t uLowNode    = sleSDChainState->getFieldU64 (sfLowNode);
    std::uint64_t uHighNode   = sleSDChainState->getFieldU64 (sfHighNode);
    TER         terResult;

    JLOG (j.trace())
        << "trustDelete: Deleting sdchain line: low";
    ...
    {
        JLOG (j.trace())
                << "trustDelete: Deleting sdchain line: high";
        terResult   = dirDelete (view,
            false,
            uHighNode,
    ...
    JLOG (j.trace()) << "trustDelete: Deleting sdchain line: state";
    view.erase(sleSDChainState);
    ...
    beast::Journal j)
{
    ...
    uint256 uIndex = getSDChainStateIndex (
        uSenderID, uReceiverID, saAmount.getCurrency ());
    auto sleSDChainState  = view.peek (keylet::line(uIndex));

    TER terResult;

    assert (!isSDA (uSenderID) && uSenderID != noAccount());
    assert (!isSDA (uReceiverID) && uReceiverID != noAccount());
    ...
    if (!sleSDChainState)
    {
        ...
        bool noSDChain = (sleAccount->getFlags() & lsfDefaultSDChain) == 0;
    ...
    {
        STAmount saBalance   = sleSDChainState->getFieldAmount (sfBalance);
        ...
        std::uint32_t const uFlags (sleSDChainState->getFieldU32 (sfFlags));
        bool bDelete = false;
        ...
        if (saBefore > zero
            ...
            && static_cast <bool> (uFlags & (!bSenderHigh ? lsfLowNoSDChain : lsfHighNoSDChain)) !=
               static_cast <bool> (view.read (keylet::account(uSenderID))->getFlags() & lsfDefaultSDChain)
            && !(uFlags & (!bSenderHigh ? lsfLowFreeze : lsfHighFreeze))
            && !sleSDChainState->getFieldAmount (
                !bSenderHigh ? sfLowLimit : sfHighLimit)
            // Sender trust limit is 0.
            && !sleSDChainState->getFieldU32 (
        ...
        sleSDChainState->setFieldAmount (sfBalance, saBalance);
        // ONLY: Adjust sdchain balance.
...
static
STAmount
sdchainTransferFee (ReadView const& view,
    ...
    beast::Journal j)
{
    if (from != issuer && to != issuer)
    {
        ...
        if (parityRate != rate)
        {
            ...
            JLOG (j.debug()) << "sdchainTransferFee:" <<
                " amount=" << amount.getFullText () <<
                " fee=" << fee.getFullText ();
...
static
TER
sdchainSend (ApplyView& view,
    AccountID const& uSenderID, AccountID const& uReceiverID,
    STAmount const& saAmount, STAmount& saActual, beast::Journal j)
{
    auto const issuer   = saAmount.getIssuer ();

    assert (!isSDA (uSenderID) && !isSDA (uReceiverID));
    assert (uSenderID != uReceiverID);

    if (uSenderID == issuer || uReceiverID == issuer || issuer == noAccount())
    {
        // Direct send: redeeming IOUs and/or sending own IOUs.
        sdchainCredit (view, uSenderID, uReceiverID, saAmount, false, j);
        saActual = saAmount;
        return tesSUCCESS;
    }
    ...
    if (!fix1141 (view.info ().parentCloseTime))
    {
        STAmount const saTransitFee = sdchainTransferFee (
    ...
    JLOG (j.debug()) << "sdchainSend> " <<
        to_string (uSenderID) <<
        " - > " << to_string (uReceiverID) <<
        " : deliver=" << saAmount.getFullText () <<
        " cost=" << saActual.getFullText ();

    TER terResult   = sdchainCredit (view, issuer, uReceiverID, saAmount, true, j);

    if (tesSUCCESS == terResult)
        terResult = sdchainCredit (view, uSenderID, issuer, saActual, true, j);
    ...
    STAmount const& saAmount, beast::Journal j)
{
    ...
    if (!saAmount.native ())
    {
        ...
        return sdchainSend (view, uSenderID, uReceiverID, saAmount, saActual, j);
    }
    ...
    /* SDA send which does not check reserve and can do pure adjustment.
    ...
    if (sender)
    {
        ...
        else
        {
            ...
            if (fv2Switch)
                view.creditHook (uSenderID, sdaAccount (), saAmount, sndBal);

            // Decrement SDA balance.
    ...
    {
        // Increment SDA balance.
        auto const rcvBal = receiver->getFieldAmount (sfBalance);
        receiver->setFieldAmount (sfBalance, rcvBal + saAmount);

        if (fv2Switch)
            view.creditHook (sdaAccount (), uReceiverID, saAmount, -rcvBal);
    ...
    beast::Journal j)
{
    ...
    if (before > zero
        ...
        // Sender reserve is set.
        && static_cast <bool> (flags & (!bSenderHigh ? lsfLowNoSDChain : lsfHighNoSDChain)) !=
           static_cast <bool> (sle->getFlags() & lsfDefaultSDChain)
...
{
    assert (!isSDA (account) && !isSDA (issue.account));
    ...
    uint256 const index = getSDChainStateIndex (
    ...
    if (!state)
    {
        ...
        bool noSDChain = (receiverAccount->getFlags() & lsfDefaultSDChain) == 0;

        return trustCreate (view, bSenderHigh, issue.account, account, index,
            receiverAccount, false, noSDChain, false, final_balance, limit, 0, 0, j);
    }
