...
{
    return getSDChainStateIndex (a, issue.account, issue.currency);
}
...
{
    return { ltSDCHAIN_STATE,
        getSDChainStateIndex(id0, id1, currency) };
}
...
payChan (AccountID const& source, AccountID const& dst, std::uint32_t seq)
{
    sha512_half_hasher h;
    using beast::hash_append;
    hash_append(h, std::uint16_t(spaceSDAUChannel));
    hash_append(h, source);
    hash_append(h, dst);
    hash_append(h, seq);
