    ...
    : STBase(name)
{
    ...
    if (isSDA (issue.currency))
        Throw<std::runtime_error> ("invalid native currency");

    issue.account.copyFrom (sit.get160 ());

    if (isSDA (issue.account))
...
STAmount::STAmount (SDAAmount const& amount)
    : mOffset (0)
    , mIsNative (true)
    , mIsNegative (amount < zero)
{
...
//
//------------------------------------------------------------------------------
SDAAmount STAmount::sda () const
{
    if (!mIsNative)
        Throw<std::logic_error> ("Cannot return non-native STAmount as SDAAmount");
...
amountFromString (Issue const& issue, std::string const& amount)
{
    ...
    // Can't specify SDA using fractional representation
    if (isSDA(issue) && match[3].matched)
        Throw<std::runtime_error> ("SDA must be specified in integral drops.");
...
amountFromJson (SField const& name, Json::Value const& v)
{
    ...
    {
        if (v.isObject ())
            Throw<std::runtime_error> ("SDA may not be specified as an object");
        issue = sdaIssue ();
    }
    else
    {
...
multiply (STAmount const& v1, STAmount const& v2, Issue const& issue)
{
    ...
    if (v1.native() && v2.native() && isSDA (issue))
    {
    ...
    bool roundUp)
{
    ...
    bool const sda = isSDA (issue);

    if (v1.native() && v2.native() && sda)
    {
    ...
    if (resultNegative != roundUp)
        canonicalizeRound (sda, amount, offset);
    ...
    {
        if (sda && *stAmountCalcSwitchover2)
        {
    ...
    Issue const& issue, bool roundUp)
{
    ...
    if (resultNegative != roundUp)
        canonicalizeRound (isSDA (issue), amount, offset);

    STAmount result (issue, amount, offset, resultNegative);
    // Control when bugfixes that require switchover dates are enabled
    if (roundUp && !resultNegative && !result && *stAmountCalcSwitchover)
    {
        if (isSDA(issue) && *stAmountCalcSwitchover2)
        {
            // return the smallest value above zero
            amount = 1;
            offset = 0;
