...
#include <sdchain/net/HTTPClient.h>
...
private:
    ...
    static Json::Value jvParseCurrencyIssuer (std::string const& strCurrencyIssuer)
    {
        ...
        if (boost::regex_match (strCurrencyIssuer, smMatch, reCurIss))
        {
            ...
            if (strIssuer.length ())
            {
                // Could confirm issuer is a valid SDChain address.
...
private:
    ...
    bool isValidJson2(Json::Value const& jv)
    {
        ...
        if (jv.isObject())
        {
            if (jv.isMember(jss::jsonrpc) && jv[jss::jsonrpc] == "2.0" &&
                jv.isMember(jss::sdchainrpc) && jv[jss::sdchainrpc] == "2.0" &&
    ...
    Json::Value parseJson2(Json::Value const& jvParams)
    {
        ...
        if (valid_parse && isValidJson2(jv))
        {
            if (jv.isObject())
            {
                ...
                jv1[jss::sdchainrpc] = jv[jss::sdchainrpc];
            ...
            for (Json::UInt j = 0; j < jv.size(); ++j)
            {
                ...
                jv1[j][jss::sdchainrpc] = jv[j][jss::sdchainrpc];
        ...
        if (jv.isMember(jss::sdchainrpc))
            jv_error[jss::sdchainrpc] = jv[jss::sdchainrpc];
    ...
    // sdchain_path_find <json> [<ledger>]
    Json::Value parseSDChainPathFind (Json::Value const& jvParams)
    {
        Json::Reader    reader;
        Json::Value     jvRequest;
...
public:
    ...
    //      {   "profile",              &RPCParser::parseProfile,               1,  9   },
            ...
            {   "sdchain_path_find",     &RPCParser::parseSDChainPathFind,        1,  2   },
...
cmdLineToJSONRPC (std::vector<std::string> const& args, beast::Journal j)
{
    ...
    if (paramsObj.isMember(jss::sdchainrpc))
        jv[jss::sdchainrpc] = paramsObj[jss::sdchainrpc];
