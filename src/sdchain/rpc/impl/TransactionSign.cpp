...
#include <sdchain/app/tx/apply.h>              // Validity::Valid
...
#include <sdchain/protocol/Sign.h>
    ...
    bool doPath)
{
    ...
    if (!tx_json.isMember (jss::Paths) && params.isMember (jss::build_path))
    {
...

        {
            ...
            if (ledger)
            {
                Pathfinder pf(std::make_shared<SDChainLineCache>(ledger),
    ...
    ProcessTransactionFn const& processTransaction)
{
    ...
    // Validate the fields in the serialized transaction.
    {
        ...
        // The Fee field must be in SDA and greater than zero.
        ...
        if (!isLegalNet (fee))
        {
            ...
            err << "Invalid " << sfFee.fieldName
                << " field.  Fees must be specified in SDA.";
            return RPC::make_error (rpcINVALID_PARAMS, err.str ());
        }
        if (fee <= 0)
        {
