...
#include <sdchain/server/SimpleWriter.h>
            ...
            Json::Value const& jv)
{
         ...
         jv[jss::command].asString() != jv[jss::method].asString()))
    {
        ...
        if (jv.isMember(jss::sdchainrpc))
            jr[jss::sdchainrpc] = jv[jss::sdchainrpc];
        ...
        std::string forwardedFor, std::string user)
{
    ...
    for (unsigned i = 0; i < size; ++i)
    {
        ...
        std::string sdchainrpc = "1.0";
        if (params.isMember(jss::sdchainrpc) && params[jss::sdchainrpc] != "1.0")
            sdchainrpc = params[jss::sdchainrpc].asString();
        /**
        ...
        if (params.isMember(jss::sdchainrpc))
           r[jss::sdchainrpc] = params[jss::sdchainrpc];
    ...
    http_request_type const& request) const
{
    ...
    if (app_.serverOkay(reason))
    {
        ...
        msg.body = "<!DOCTYPE html><html><head><title>" + systemName() +
            " Test page for sdchaind</title></head><body><h1>" +
                systemName() + " Test</h1><p>This page shows sdchaind http(s) "
