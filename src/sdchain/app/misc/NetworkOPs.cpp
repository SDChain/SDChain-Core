...
#include <sdchain/consensus/Consensus.h>
#include <sdchain/app/consensus/RCLConsensus.h>
#include <sdchain/app/consensus/RCLValidations.h>
...
#include <sdchain/consensus/ConsensusParms.h>
#include <sdchain/app/ledger/LedgerToJson.h>
#include <sdchain/app/ledger/LocalTxs.h>
...
#include <sdchain/app/misc/impl/AccountTxPaging.h>
...
#include <sdchain/overlay/predicates.h>
        ...
        do
        {
            for (auto const& uDirEntry : sleNode->getFieldV256 (sfIndexes))
            {
                ...
                case ltSDCHAIN_STATE:
                    if (!jvObjects.isMember (jss::sdchain_lines))
                    {
                        jvObjects[jss::sdchain_lines] =
                                Json::Value (Json::arrayValue);
                    }

                    jvObjects[jss::sdchain_lines].append (sleCur->getJson (0));
                    break;
    ...
    if (!human)
    {
        ...
        if (escalationMetrics)
        {
            ...
            /* Json::Value doesn't support uint64, so clamp to max
                ...
                probably isn't enough extant SDA to drive the factor
                that high.
            */
