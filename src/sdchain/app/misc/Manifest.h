...
#ifndef SDCHAIN_APP_MISC_MANIFEST_H_INCLUDED
#define SDCHAIN_APP_MISC_MANIFEST_H_INCLUDED
...
/*
    ...
    Suppose the secret keys installed on a SDChain validator are compromised.  Not
    only do you have to generate and install new key pairs on each validator,
    EVERY sdchaind needs to have its config updated with the new public keys, and
    ...
    signed by the master key.  When a sdchaind peer receives the new manifest,
