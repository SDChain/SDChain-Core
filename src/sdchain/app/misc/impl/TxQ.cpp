    ...
    TxQ::Setup const& setup)
{
    ...
    // happen.
    return mulDiv(tx[sfFee].sda().drops(),
        baseRefLevel,
...
/*
    ...
    0. Is `featureFeeEscalation` enabled?
        ...
        No: Fallback to `sdchain::apply`. Stop.
        ...
        ApplyFlags flags, beast::Journal j)
{
    ...
    if (!allowEscalation)
    {
        return sdchain::apply(app, view, *tx, flags, j);
    }
    ...
    struct MultiTxn
    {
        ...
        SDAAmount fee = beast::zero;
        SDAAmount potentialSpend = beast::zero;
        ...
        if (sle)
        {
            ...
            if (multiTxn)
            {
                /* Check if the total fees in flight are greater
                    ...
                    Assume: Minimum account reserve is 20 SDA.
                    Example 1: If I have 1,000,000 SDA, I can queue
                        a transaction with a 1,000,000 SDA fee. In
                        the meantime, some other transaction may
                        lower my balance (eg. taking an offer). When
                        the transaction executes, I will either
                        spend the 1,000,000 SDA, or the transaction
                        will get stuck in the queue with a
                        `terINSUF_FEE_B`.
                    Example 2: If I have 1,000,000 SDA, and I queue
                        10 transactions with 0.1 SDA fee, I have 1 SDA
                        in flight. I can now queue another tx with a
                        999,999 SDA fee. When the first 10 execute,
                        ...
                        999,999 SDA, or get stuck in the queue.
                    Example 3: If I have 1,000,000 SDA, and I queue
                        7 transactions with 3 SDA fee, I have 21 SDA
                ...
                */
                auto const balance = (*sle)[sfBalance].sda();
                auto totalFee = multiTxn->fee;
                if (multiTxn->includeCurrentFee)
                    totalFee += (*tx)[sfFee].sda();
                if (totalFee >= balance ||
