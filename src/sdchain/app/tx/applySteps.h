...
#ifndef SDCHAIN_TX_APPLYSTEPS_H_INCLUDED
#define SDCHAIN_TX_APPLYSTEPS_H_INCLUDED
...
struct TxConsequences
{
    ...
    SDAAmount const potentialSpend;

    /// Constructor
    TxConsequences(ConsequenceCategory const category_,
        SDAAmount const fee_, SDAAmount const spend_)
        : category(category_)
        , fee(fee_)
...
/** Determine the SDA balance consequences if a transaction
    consumes the maximum SDA allowed.
