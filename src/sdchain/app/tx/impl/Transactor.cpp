...
preflight1 (PreflightContext const& ctx)
{
    ...
    if (!fee.native () || fee.negative () || !isLegalAmount (fee.sda ()))
    {
...
Transactor::checkFee (PreclaimContext const& ctx, std::uint64_t baseFee)
{
    ...
    auto const feeDue = sdchain::calculateFee(ctx.app,
...
TER Transactor::payFee ()
{
    ...
    // VFALCO Should we call view().rawDestroySDA() here as well?
...
TER Transactor::apply ()
{
    ...
    if (sle)
    {
        mPriorBalance   = STAmount ((*sle)[sfBalance]).sda ();
...
void
Transactor::claimFee (SDAAmount& fee, TER terResult, std::vector<uint256> const& removedOffers)
{
    ctx_.discard();
    ...
    auto const balance = txnAcct->getFieldAmount (sfBalance).sda ();
...
Transactor::operator()()
{
    ...
    auto fee = ctx_.tx.getFieldAmount(sfFee).sda ();
