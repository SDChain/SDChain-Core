...
static
SDAAmount accountFundsHelper (ReadView const& view,
    AccountID const& id,
    SDAAmount const& amtDefault,
    Issue const& issue,
    FreezeHandling freezeHandling,
    beast::Journal j)
{
    return toAmount<SDAAmount> (
        accountHolds (view, id, issue.currency, issue.account, freezeHandling, j));
}
...
template class FlowOfferStream<SDAAmount, IOUAmount>;
template class FlowOfferStream<IOUAmount, SDAAmount>;

template class TOfferStreamBase<STAmount, STAmount>;
template class TOfferStreamBase<IOUAmount, IOUAmount>;
template class TOfferStreamBase<SDAAmount, IOUAmount>;
template class TOfferStreamBase<IOUAmount, SDAAmount>;
}
