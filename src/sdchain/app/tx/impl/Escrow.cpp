...
#include <sdchain/app/tx/impl/Escrow.h>
...
/*
    ...
    of SDA in its own ledger entry, until the escrow process
    ...
    sequestered SDA. If the escrow is, instead, canceled,
    the account which created the escrow will receive the
    sequestered SDA back instead.
    ...
    EscrowCreate
        ...
        Validation rules:
            ...
            sfCancelAfter
                ...
                specified time (seconds after the SDChain epoch).
...
EscrowCreate::preflight (PreflightContext const& ctx)
{
    ...
    {
        using namespace sdchain::cryptoconditions;
...
EscrowCreate::doApply()
{
    ...
    {
        auto const balance = STAmount((*sle)[sfBalance]).sda();
        ...
        if (balance < reserve + STAmount(ctx_.tx[sfAmount]).sda())
            return tecUNFUNDED;
    }
