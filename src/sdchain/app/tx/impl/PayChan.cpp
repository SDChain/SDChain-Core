...
#include <sdchain/app/tx/impl/PayChan.h>
...
/*
    PaymentChannel
        ...
        Payment channels permit off-ledger checkpoints of SDA payments flowing
        in a single direction. A channel sequesters the owner's SDA in its own
    ...
    PaymentChannelCreate
        ...
        Amount
            The amount of SDA to deposit in the channel immediately.
        SettleDelay
        ...
        DestinationTag (optional)
            ...
            Wallet to be mapped back onto the SDChain ledger. The destination tag
        ...
        SourceTag (optional)
            ...
            to be mapped back onto the SDChain ledger. Source tags are similar to
    ...
    PaymentChannelClaim
        ...
        Channel
            The 256-bit ID of the channel.
        Balance (optional)
            The total amount of SDA delivered after this claim is processed (optional, not
            needed if just closing).
        Amount (optional)
            The amount of SDA the signature is for (not needed if equal to Balance or just
            closing the line).
        Signature (optional)
...
PayChanCreate::preflight (PreflightContext const& ctx)
{
    ...
    if (!isSDA (ctx.tx[sfAmount]) || (ctx.tx[sfAmount] <= beast::zero))
...
PayChanCreate::preclaim(PreclaimContext const &ctx)
{
...

    {
        ...
        if ((*sled)[sfFlags] & lsfDisallowSDA)
            return tecNO_TARGET;
    }
...
PayChanClaim::preflight (PreflightContext const& ctx)
{
    ...
    if (bal && (!isSDA (*bal) || *bal <= beast::zero))
        return temBAD_AMOUNT;

    auto const amt = ctx.tx[~sfAmount];
    if (amt && (!isSDA (*amt) || *amt <= beast::zero))
        return temBAD_AMOUNT;
    ...
    if (auto const sig = ctx.tx[~sfSignature])
    {
        ...
        auto const reqBalance = bal->sda ();
        auto const authAmt = amt ? amt->sda() : reqBalance;
...
PayChanClaim::doApply()
{
    ...
    {
        auto const chanBalance = slep->getFieldAmount (sfBalance).sda ();
        auto const chanFunds = slep->getFieldAmount (sfAmount).sda ();
        auto const reqBalance = ctx_.tx[sfBalance].sda ();
        ...
        if (txAccount == src && ((*sled)[sfFlags] & lsfDisallowSDA))
            return tecNO_TARGET;

        (*slep)[sfBalance] = ctx_.tx[sfBalance];
        SDAAmount const reqDelta = reqBalance - chanBalance;
