...
#include <sdchain/app/tx/impl/Payment.h>
...
Payment::calculateMaxSpend(STTx const& tx)
{
    if (tx.isFieldPresent(sfSendMax))
    {
        ...
        return sendMax.native() ? sendMax.sda() : beast::zero;
    }
    /* If there's no sfSendMax in SDA, and the sfAmount isn't
    in SDA, then the transaction can not send SDA. */
    auto const& saDstAmount = tx.getFieldAmount(sfAmount);
    return saDstAmount.native() ? saDstAmount.sda() : beast::zero;
}
...
Payment::preflight (PreflightContext const& ctx)
{
    ...
    bool const defaultPathsAllowed = !(uTxFlags & tfNoSDChainDirect);
    ...
    // isZero() is SDA.  FIX!
    bool const bSDADirect = uSrcCurrency.isZero () && uDstCurrency.isZero ();
    ...
    if (bSDADirect && bMax)
    {
        ...
        JLOG(j.trace()) << "Malformed transaction: " <<
            "SendMax specified for SDA to SDA.";
        return temBAD_SEND_SDA_MAX;
    }
    if (bSDADirect && bPaths)
    {
        // SDA is sent without paths.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "Paths specified for SDA to SDA.";
        return temBAD_SEND_SDA_PATHS;
    }
    if (bSDADirect && partialPaymentAllowed)
    {
        // Consistent but redundant transaction.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "Partial payment specified for SDA to SDA.";
        return temBAD_SEND_SDA_PARTIAL;
    }
    if (bSDADirect && limitQuality)
    {
        // Consistent but redundant transaction.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "Limit quality specified for SDA to SDA.";
        return temBAD_SEND_SDA_LIMIT;
    }
    if (bSDADirect && !defaultPathsAllowed)
    {
        // Consistent but redundant transaction.
        JLOG(j.trace()) << "Malformed transaction: " <<
            "No sdchain direct specified for SDA to SDA.";
        return temBAD_SEND_SDA_NO_DIRECT;
    }
...
{
    // SDChain if source or destination is non-native or if there are paths.
    ...
    {
        // SDChain payment with at least one intermediate step and uses
        // transitive balances.
...
Payment::doApply ()
{
    ...
    bool const bSDChain = paths || sendMax || !saDstAmount.native ();
    // XXX Should sendMax be sufficient to imply sdchain?
    ...
    if (bSDChain)
    {
        ...
        path::SDChainCalc::Output rc;
        {
            PaymentSandbox pv(&view());
            JLOG(j_.debug())
                << "Entering SDChainCalc in payment: " << ctx_.tx.getTransactionID();
        ...
        // Because of its overhead, if SDChainCalc
    ...
    else
    {
        ...
        auto const mmm = std::max(reserve,
            ctx_.tx.getFieldAmount (sfFee).sda ());

        if (mPriorBalance < saDstAmount.sda () + mmm)
        {
            // Vote no. However the transaction might succeed, if applied in
            // a different order.
            JLOG(j_.trace()) << "Delay transaction: Insufficient funds: " <<
                " " << to_string (mPriorBalance) <<
                " / " << to_string (saDstAmount.sda () + mmm) <<
                " (" << to_string (reserve) << ")";
