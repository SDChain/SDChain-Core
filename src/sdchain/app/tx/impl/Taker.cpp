...
#include <sdchain/app/tx/impl/Taker.h>
    ...
    , journal_ (journal)
{
    ...
    // flavor we expect:
    assert (cross_type != CrossType::SdaToIou ||
        (isSDA (issue_in ()) && !isSDA (issue_out ())));

    assert (cross_type != CrossType::IouToSda ||
        (!isSDA (issue_in ()) && isSDA (issue_out ())));

    // And make sure we're not crossing SDA for SDA
    assert (!isSDA (issue_in ()) || !isSDA (issue_out ()));
    ...
    Rate const& rate_out)
{
    ...
    // of an autobridge.
    if (cross_type_ == CrossType::SdaToIou && (remaining_.in < f.order.in))
    {
    ...
    Rate const& rate_in)
{
    ...
    if (!sell_ && cross_type_ == CrossType::IouToSda)
    {
...
BasicTaker::do_cross (Amounts offer, Quality quality, AccountID const& owner)
{
    ...
    if (cross_type_ == CrossType::SdaToIou)
    {
        result = flow_sda_to_iou (offer, quality, owner_funds, taker_funds,
            out_rate (owner, account ()));
    }
    else if (cross_type_ == CrossType::IouToSda)
    {
        result = flow_iou_to_sda (offer, quality, owner_funds, taker_funds,
    ...
    Amounts offer2, Quality quality2, AccountID const& owner2)
{
    ...
    // The amount available to flow via SDA is the amount that the owner of the
    ...
    // of SDA that can flow between the two legs is, essentially, infinite
    // since all the owner is doing is taking out SDA of his left pocket
    // and putting it in his right pocket. In that case, we set the available
    // SDA to the largest of the two offers.
    auto sda_funds = get_funds (owner1, offer1.out);

    if (owner1 == owner2)
    {
        JLOG(journal_.trace()) <<
            "The bridge endpoints are owned by the same account.";
        sda_funds = std::max (offer1.out, offer2.in);
    }
    ...
    if (auto stream = journal_.debug())
    {
        ...
        stream << "      sda: " << format_amount (sda_funds);
    }
    ...
    auto flow1 = flow_iou_to_sda (offer1, quality1, sda_funds, leg1_in_funds, leg1_rate);

    if (!flow1.sanity_check ())
        Throw<std::logic_error> ("Computed flow1 fails sanity check.");

    auto flow2 = flow_sda_to_iou (offer2, quality2, leg2_out_funds, sda_funds, leg2_rate);
    ...
    // equalize them, so that the amount of SDA that flows out of the first leg
    // is the same as the amount of SDA that flows into the second leg. We take
...
TER Taker::transferSDA (
    AccountID const& from,
    AccountID const& to,
    STAmount const& amount)
{
    if (!isSDA (amount))
        Throw<std::logic_error> ("Using transferSDA with IOU");
    ...
    return sdchain::transferSDA (view_, from, to, amount, journal_);
}
...
{
    if (isSDA (amount))
        Throw<std::logic_error> ("Using redeemIOU with SDA");
    ...
    auto ret = sdchain::redeemIOU (view_, account, amount, issue, journal_);
...
{
    if (isSDA (amount))
        Throw<std::logic_error> ("Using issueIOU with SDA");
    ...
    return sdchain::issueIOU (view_, account, amount, issue, journal_);
}
...
Taker::fill (BasicTaker::Flow const& flow, Offer& offer)
{
    ...
    if (cross_type () != CrossType::SdaToIou)
    {
        assert (!isSDA (flow.order.in));
    ...
    else
    {
        ...
        if (result == tesSUCCESS)
            result = transferSDA (account (), offer.owner (), flow.order.in);
    }

    // Now send funds from the account whose offer we're taking
    if (cross_type () != CrossType::IouToSda)
    {
        assert (!isSDA (flow.order.out));
    ...
    else
    {
        ...
        if (result == tesSUCCESS)
            result = transferSDA (offer.owner (), account (), flow.order.out);
    }
    ...
    BasicTaker::Flow const& flow2, Offer& leg2)
{
    ...
    // leg1 to leg2: bridging over SDA
    if (result == tesSUCCESS)
        result = transferSDA (leg1.owner (), leg2.owner (), flow1.order.out);
...
{
    // In direct crossings, at least one leg must not be SDA.
    if (isSDA (offer.amount ().in) && isSDA (offer.amount ().out))
        return tefINTERNAL;
...
{
    // In bridged crossings, SDA must can't be the input to the first leg
    // or the output of the second leg.
    if (isSDA (leg1.amount ().in) || isSDA (leg2.amount ().out))
        return tefINTERNAL;
...
Taker::calculateRate (
    ApplyView const& view,
        AccountID const& issuer,
            AccountID const& account)
{
    return isSDA (issuer) || (account == issuer)
        ? parityRate
