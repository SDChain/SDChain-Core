...
#include <sdchain/app/tx/impl/CreateOffer.h>
...
CreateOffer::preflight (PreflightContext const& ctx)
{
    ...
    {
        JLOG(j.debug()) <<
            "Malformed offer: redundant (SDA for SDA)";
        return temBAD_OFFER;
    }
    ...
    // We don't allow a non-native currency to use the currency code SDA.
...
CreateOffer::reachedOfferCrossingLimit (Taker const& taker) const
{
    ...
    // The crossing limit is part of the SDChain protocol and
    ...
    NetClock::time_point const when)
{
    ...
    assert (!isSDA (takerAmount.in) && !isSDA (takerAmount.out));

    if (isSDA (takerAmount.in) || isSDA (takerAmount.out))
        Throw<std::logic_error> ("Bridging with SDA and an endpoint.");
    ...
    OfferStream offers_leg1 (view, view_cancel,
        Book (taker.issue_in (), sdaIssue ()),
        when, stepCounter_, j_);

    OfferStream offers_leg2 (view, view_cancel,
        Book (sdaIssue (), taker.issue_out ()),
        when, stepCounter_, j_);
    ...
    Amounts const& takerAmount)
{
    ...
    //
    // We check this in preclaim, but when selling SDA
    ...
    Amounts const& takerAmount)
{
    try
    {
        ...
        //
        // We check this in preclaim, but when selling SDA charged fees can
        ...
        // of the takerAmount currencies are SDA then we cross through an
        // additional path with SDA as the intermediate between two books.
        // This second path we have to build ourselves.
        STPathSet paths;
        if (!takerAmount.in.native() & !takerAmount.out.native())
        {
            STPath path;
            path.emplace_back (boost::none, sdaCurrency(), boost::none);
...
CreateOffer::preCompute()
{
    ...
    if (pays_sda && !gets_sda)
        cross_type_ = CrossType::IouToSda;
    else if (gets_sda && !pays_sda)
        cross_type_ = CrossType::SdaToIou;
...
CreateOffer::applyGuts (Sandbox& sb, Sandbox& sbCancel)
{
    ...
    {
        SDAAmount reserve = ctx_.view().fees().accountReserve(
