...
The pathfinding request is identified by category, SDA to SDA, SDA to
non-SDA, non-SDA to SDA, same currency non-SDA to non-SDA, cross-currency
non-SDA to non-SDA.  For each category, there is a table of paths that the
...
Pathfinder::Pathfinder (
    ...
    :   mSrcAccount (uSrcAccount),
        ...
        mEffectiveDst (isSDA(saDstAmount.getIssuer ()) ?
        ...
        mSrcAmount (srcAmount.value_or(STAmount({uSrcCurrency,
            uSrcIssuer.value_or(isSDA(uSrcCurrency) ?
                sdaAccount() : uSrcAccount)}, 1u, 0, true))),
...
{
    assert (! uSrcIssuer || isSDA(uSrcCurrency) == isSDA(uSrcIssuer.get()));
}
...
bool Pathfinder::findPaths (int searchLevel)
{
    ...
    auto currencyIsSDA = isSDA (mSrcCurrency);

    bool useIssuerAccount
            = mSrcIssuer && !currencyIsSDA && !isSDA (*mSrcIssuer);
    auto& account = useIssuerAccount ? *mSrcIssuer : mSrcAccount;
    auto issuer = currencyIsSDA ? AccountID() : account;
    ...
    bool bSrcSda = isSDA (mSrcCurrency);
    bool bDstSda = isSDA (mDstAmount.getCurrency());
    ...
    if (! mLedger->exists (keylet::account (mDstAccount)))
    {
        ...
        {
            JLOG (j_.debug())
                    << "New account not being funded in SDA ";
            return false;
        }
    ...
    {
        // SDA -> SDA
        JLOG (j_.debug()) << "SDA to SDA payment";
        paymentType = pt_SDA_to_SDA;
    }
    else if (bSrcSda)
    {
        // SDA -> non-SDA
        JLOG (j_.debug()) << "SDA to non-SDA payment";
        paymentType = pt_SDA_to_nonSDA;
    }
    else if (bDstSda)
    {
        // non-SDA -> SDA
        JLOG (j_.debug()) << "non-SDA to SDA payment";
        paymentType = pt_nonSDA_to_SDA;
    }
    else if (mSrcCurrency == mDstAmount.getCurrency ())
    {
        // non-SDA -> non-SDA - Same currency
        JLOG (j_.debug()) << "non-SDA to non-SDA - same currency";
        paymentType = pt_nonSDA_to_same;
    }
    else
    {
        // non-SDA to non-SDA - Different currency
        JLOG (j_.debug()) << "non-SDA to non-SDA - cross currency";
        paymentType = pt_nonSDA_to_nonSDA;
    }
    ...
    uint64_t& qualityOut) const    // OUT: The returned initial quality
{
    ...
    path::SDChainCalc::Input rcInput;
    ...
    try
    {
        ...
        auto rc = path::SDChainCalc::sdchainCalculate (
            sandbox,
            mSrcAmount,
            minDstAmount,
            mDstAccount,
        ...
        if (! convert_all_)
        {
            ...
            rcInput.partialPaymentAllowed = true;
            rc = path::SDChainCalc::sdchainCalculate (
                sandbox,
                mSrcAmount,
...
static bool isDefaultPath (STPath const& path)
{
    ...
    // https://forum.sdchain.io/viewtopic.php?f=2&t=8206&start=10#p57713
    //
    // JoelKatz writes:
    ...
    AccountID const& srcIssuer)
{
    ...
    const bool issuerIsSender = isSDA (mSrcCurrency) || (srcIssuer == mSrcAccount);
    ...
    AccountID const& dstAccount)
{
    ...
    if (!bFrozen)
    {
        ...
        for (auto const& item : mRLCache->getSDChainLines (account))
        {
            SDChainState* rspEntry = (SDChainState*) item.get ();
            ...
            else if (rspEntry->getNoSDChainPeer ())
            {
...
STPathSet& Pathfinder::addPathsForType (PathType const& pathType)
{
    ...
    case nt_SDA_BOOK:
        addLinks (parentPaths, pathsOut, afADD_BOOKS | afOB_SDA);
        break;
...
bool Pathfinder::isNoSDChain (
    AccountID const& fromAccount,
    AccountID const& toAccount,
    Currency const& currency)
{
    auto sleSDChain = mLedger->read(keylet::line(
        toAccount, fromAccount, currency));

    auto const flag ((toAccount > fromAccount)
                     ? lsfHighNoSDChain : lsfLowNoSDChain);

    return sleSDChain && (sleSDChain->getFieldU32 (sfFlags) & flag);
}

// Does this path end on an account-to-account link whose last account has
// set "no sdchain" on the link?
bool Pathfinder::isNoSDChainOut (STPath const& currentPath)
{
    ...
    // no sdchain on the output. A path with no sdchain on its output can't be
    // followed by a link with no sdchain on its input.
    auto const& fromAccount = (currentPath.size () == 1)
        ? mSrcAccount
        : (currentPath.end () - 2)->getAccountID ();
    auto const& toAccount = endElement.getAccountID ();
    return isNoSDChain (fromAccount, toAccount, endElement.getCurrency ());
}
    ...
    int addFlags)
{
    ...
    bool const bOnSDA = uEndCurrency.isZero ();
    ...
    if (addFlags & afADD_ACCOUNTS)
    {
        ...
        if (bOnSDA)
        {
            ...
            { // non-default path to SDA destination
                JLOG (j_.trace())
        ...
        else
        {
            ...
            if (sleEnd)
            {
                ...
                bool const bIsNoSDChainOut (
                    isNoSDChainOut (currentPath));
                bool const bDestOnly (
                    addFlags & afAC_LAST);

                auto& sdchainLines (mRLCache->getSDChainLines (uEndAccount));

                AccountCandidates candidates;
                candidates.reserve (sdchainLines.size ());

                for (auto const& item : sdchainLines)
                {
                    auto* rs = dynamic_cast<SDChainState const *> (item.get ());
                    if (!rs)
                    {
                        JLOG (j_.error())
                                << "Couldn't decipher SDChainState";
                        continue;
                    }
                        ...
                        !currentPath.hasSeen (acct, uEndCurrency, acct))
                    {
                        ...
                        else if (bIsNoSDChainOut && rs->getNoSDChain ())
                        {
    ...
    {
        // add order books
        if (addFlags & afOB_SDA)
        {
            // to SDA only
            if (!bOnSDA && app_.getOrderBookDB ().isBookToSDA (
            ...
            for (auto const& book : books)
            {
                     ...
                     (book->getCurrencyOut () == mDstAmount.getCurrency ())))
                {
                    ...
                    { // to SDA
                        ...
                        {
                            // destination is SDA, add account and path is
                            // complete
                            JLOG (j_.trace())
                        ...
                        book->getIssuerOut ()))
                    {
                            ...
                            (newPath[newPath.size() - 2].isOffer ()))
                        {
                            ...
                            newPath[newPath.size() - 1] = STPathElement (
                                ...
                                sdaAccount(), book->getCurrencyOut(),
                                book->getIssuerOut());
                        }
                        else
                        {
...
Pathfinder::PathType makePath (char const *string)
{
    ...
    while (true)
    {
        switch (*string++)
        {
            ...
            case 'x': // sda book
                ret.push_back (Pathfinder::nt_SDA_BOOK);
                break;
...
void Pathfinder::initPathTable ()
{
    ...
    fillPaths(
        pt_nonSDA_to_SDA, {
            {1, "sxd"},       // gateway buys SDA
            {2, "saxd"},      // source -> gateway -> book(SDA) -> dest
            {6, "saaxd"},
            {7, "sbxd"},
            {8, "sabxd"},
            {9, "sabaxd"}
    ...
    // non-SDA to non-SDA (same currency)
    fillPaths(
        pt_nonSDA_to_same,  {
            ...
            {6, "sxfad"},
            {6, "safad"},
            {6, "saxfd"},   // source -> gateway -> book to SDA -> book ->
                            // destination
            {6, "saxfad"},
