...
bool PathRequest::isValid (std::shared_ptr<SDChainLineCache> const& crCache)
{
    ...
    if (! sleDest)
    {
        ...
        {
            // Only SDA can be send to a non-existent account.
    ...
    {
        bool const disallowSDA (
            sleDest->getFlags() & lsfDisallowSDA);

        auto usDestCurrID = accountDestCurrencies (
                *raDstAccount, crCache, ! disallowSDA);
...
PathRequest::getPathFinder(std::shared_ptr<SDChainLineCache> const& cache,
...
bool
PathRequest::findPaths (std::shared_ptr<SDChainLineCache> const& cache,
    int const level, Json::Value& jvArray)
{
        ...
        for (auto const& c : currencies)
        {
            if (! sameAccount || c != saDstAmount.getCurrency())
            {
                ...
                sourceCurrencies.insert(
                    {c, c.isZero() ? sdaAccount() : *raSrcAccount});
            }
        }
    }
    ...
    for (auto const& issue : sourceCurrencies)
    {
        ...
        auto& sourceAccount = ! isSDA(issue.account)
            ? issue.account
        ...
        JLOG(m_journal.debug()) << iIdentifier
            << " Paths found, calling sdchainCalc";
        ...
        if (rc.result () == tesSUCCESS)
        {
            ...
            if (hasCompletion ())
            {
                // Old sdchain_path_find API requires this
        ...
        {
            JLOG(m_journal.debug()) << iIdentifier << " sdchainCalc returns "
                << transHuman(rc.result());
        }
    }
...
Json::Value PathRequest::doUpdate(
    std::shared_ptr<SDChainLineCache> const& cache, bool fast)
{
    using namespace std::chrono;
    JLOG(m_journal.debug()) << iIdentifier
    ...
    if (hasCompletion ())
    {
        // Old sdchain_path_find API gives destination_currencies
