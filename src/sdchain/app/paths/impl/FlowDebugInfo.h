...
#ifndef SDCHAIN_PATH_IMPL_FLOWDEBUGINFO_H_INCLUDED
#define SDCHAIN_PATH_IMPL_FLOWDEBUGINFO_H_INCLUDED
    ...
    to_string (bool writePassInfo) const
    {
        ...
        if (writePassInfo)
        {
            ...
            auto writeIouAmtList = [&write_list](
                std::vector<EitherAmount> const& amts, char delim=';') {
                auto get_val = [](EitherAmount const& a) -> std::string {
                    return sdchain::to_string (a.iou);
                };
                write_list (amts, get_val, delim);
            };
            ...
            auto writeNestedSdaAmtList = [&ostr, &writeSdaAmtList](
                std::vector<std::vector<EitherAmount>> const& amts) {
                ostr << '[';
                if (!amts.empty ())
                {
                    writeSdaAmtList(amts[0], '|');
                    for (size_t i = 1, e = amts.size (); i < e; ++i)
                    {
                        ostr << ';';
                        writeSdaAmtList(amts[i], '|');
                    }
                }
                ostr << ']';
            };
            ...
            if (passInfo.nativeIn)
                writeSdaAmtList (passInfo.in);
            else
                writeIouAmtList (passInfo.in);
            ostr << ", out_pass: ";
            if (passInfo.nativeOut)
                writeSdaAmtList (passInfo.out);
            else
                ...
                !passInfo.liquiditySrcIn.back ().empty ())
            {
                ostr << ", l_src_in: ";
                if (passInfo.nativeIn)
                    writeNestedSdaAmtList (passInfo.liquiditySrcIn);
                else
                    writeNestedIouAmtList (passInfo.liquiditySrcIn);
                ostr << ", l_src_out: ";
                if (passInfo.nativeOut)
                    writeNestedSdaAmtList (passInfo.liquiditySrcOut);
                else
...
{
    return {sb.balanceChanges (rv), sb.sdaDestroyed ()};
}
...
balanceDiffsToString (boost::optional<BalanceDiffs> const& bd)
{
    ...
    auto const& sdaDestroyed = bd->second;
    std::ostringstream ostr;
    ostr << ", sdaDestroyed: " << to_string (sdaDestroyed);
    ostr << ", balanceDiffs: ";
