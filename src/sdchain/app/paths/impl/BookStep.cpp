...
#include <sdchain/app/tx/impl/OfferStream.h>
...
public:
        ...
        bool prevStepRedeems) const
    {
        ...
        auto rate = [&](AccountID const& id) {
            if (isSDA(id) || id == this->strandDst_)
                return parityRate;
    ...
    Callback& callback) const
{
    ...
    while (offers.step ())
    {
        ...
        // An account can always own SDA or their own IOUs.
        if (flowCross &&
            (!isSDA (offer.issueIn().currency)) &&
...
template <class TIn, class TOut, class TDerived>
static
bool equalHelper (Step const& step, sdchain::Book const& book)
{
...
bool bookStepEqual (Step const& step, sdchain::Book const& book)
{
    bool const inSDA = isSDA (book.in.currency);
    bool const outSDA = isSDA (book.out.currency);
    if (inSDA && outSDA)
        return equalHelper<SDAAmount, SDAAmount,
            BookPaymentStep<SDAAmount, SDAAmount>> (step, book);
    if (inSDA && !outSDA)
        return equalHelper<SDAAmount, IOUAmount,
            BookPaymentStep<SDAAmount, IOUAmount>> (step, book);
    if (!inSDA && outSDA)
        return equalHelper<IOUAmount, SDAAmount,
            BookPaymentStep<IOUAmount, SDAAmount>> (step, book);
    if (!inSDA && !outSDA)
...
make_BookStepIX (
    ...
    Issue const& in)
{
    return make_BookStepHelper<IOUAmount, SDAAmount> (ctx, in, sdaIssue());
}
...
make_BookStepXI (
    ...
    Issue const& out)
{
    return make_BookStepHelper<SDAAmount, IOUAmount> (ctx, sdaIssue(), out);
}
