    ...
    while (resultCode == tesSUCCESS && saInAct + saInFees < saInReq)
    {
        ...
        else if (resultCode == tesSUCCESS)
        {
            ...
            if (!isSDA(nextNode().account_))
            {
                ...
                // Output fees: none as SDA or the destination account is the
                // issuer.
                ...
                // Output: Debit offer owner, send SDA or non-XPR to next
                // account.
            ...
            else
            {
                ...
                // fees).
                auto const& id = isSDA(node().issue_) ?
                        sdaAccount() : node().issue_.account;
            ...
            // is redundant.
            if (isSDA (previousNode().issue_.currency)
                || uInAccountID != node().offerOwnerAccount_)
            {
                auto id = !isSDA(previousNode().issue_.currency) ?
                        uInAccountID : sdaAccount();
                resultCode = accountSend(view(),
                    id,
                    node().offerOwnerAccount_,
                    saInPassAct,
