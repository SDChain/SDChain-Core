...
// the sdchaind server-server use case, where we only need MITM
...
get_context (std::string const& cipherList)
{
    ...
    // These are the raw DH parameters that SDChain Alliance has
    // chosen for SDChain, in the binary format needed by
    // d2i_DHparams.
    //
